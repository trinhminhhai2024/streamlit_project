...
vocabs = load_vocab(file_path='sources/vocab.txt')
...
def levenshtein_distance(token1, token2):
    ...
    for t1 in range(1, len(token1) + 1):
        for t2 in range(1, len(token2) + 1):
            distances[t1][t2] = calculate_min_distance(
                distances, t1, t2, token1, token2)

    return distances[len(token1)][len(token2)]


def calculate_min_distance(distances, t1, t2, token1, token2):
    if token1[t1-1] == token2[t2-1]:
        return distances[t1 - 1][t2 - 1]
    else:
        insert = distances[t1][t2 - 1]
        delete = distances[t1 - 1][t2]
        replace = distances[t1 - 1][t2 - 1]
